...
class Particle:
    ...
    def draw_scaled(self, mouse_x, mouse_y):
        scaled_x = int(mouse_x + (self.position.x - mouse_x) * resources.constants.SCALE)
        scaled_y = int(mouse_y + (self.position.y - mouse_y) * resources.constants.SCALE)
        circle_center = (scaled_x, scaled_y)
        pygame.draw.circle(resources.constants.SCREEN, self.color, circle_center, int(self.radius * resources.constants.SCALE))
        ...
        if self.selected:
            ...
            line_height = 20  
            text_y = scaled_y + int(self.radius * resources.constants.SCALE) + 10  
            for i, line in enumerate(stats_lines):
                stats_text = resources.constants.FONT.render(line, True, (255, 255, 255))
                stats_text_rect = stats_text.get_rect(center=(scaled_x, text_y + i * line_height))
                resources.constants.SCREEN.blit(stats_text, stats_text_rect)
